...
class App extends Component {
  ...
  componentDidMount = async () => {
    try {
      ...
      const instance = new web3.eth.Contract(
        ...
        "0x0Bb909b7c3817F8fB7188e8fbaA2763028956E30"
      );
    ...
    var url = new URL("https://api.compound.finance/api/v2/ctoken");
    ...
    fetch(url).then((response) =>
      response
        ...
        .then((res) => {
          ...
          let tokenKeys = [...this.state.tokenKeys];

          for (var i = 0; i < 3; i++) {
            var result = res.data.cToken.filter((obj) => {
              return obj.symbol === this.state.tokenKeys[i];
            });

            tokenKeys[i] = result;
          }

          this.setState({ tokenKeys });

          console.log(this.state.tokenKeys[0][0].exchange_rate.value);

          this.setState({ penis: 1 });

          var cock = Swap(
            this.state.tokenKeys[0][0].exchange_rate.value,
            this.state.tokenKeys[1][0].exchange_rate.value,
            this.state.tokenKeys[0][0].supply_rate.value,
            this.state.tokenKeys[1][0].supply_rate.value
          );
          console.log(cock);
        })
    );
  ...
  buttonClick = async (e) => {
    ...
    await contract.methods.set(Number(this.state.val)).send({ from: account });
  };
  ...
  usdtClick = async (e) => {
    ...
    const cToken = new web3.eth.Contract(cTokenAbiJson, cTokenContractAddress);
    await cToken.methods
      .transfer("0xa0df350d2637096571F7A701CBc1C5fdE30dF76A", 10 * 10 ** 8)
      .send({ from: account });
  ...
  selectChange = (e) => {
    this.setState({ scalar: parseInt(e) });
  };

  changeTab = (e) => {
    this.setState({ activeTab: parseInt(e) });
  };
  ...
  render() {
    ...
    if (!this.state.ctoken || !this.state.penis) {
      return null;
    }
    return (
      <Container>
        <div className="App">
          ...
          <div>
            <Row gutter={64}>
              <Col span={8}>
                <Card title="USDT | DAI">
                  <Input id="standard-basic" onChange={this.Change} />

                  <Button onClick={this.usdtClick}>Click me cock</Button>

                  <Button onClick={this.Get}>hi</Button>
                </Card>
              </Col>
              <Col span={8}>
                <Card title="DAI | USDC">
              ...
              <Col span={8}>
                <Card title="USDC | USDT">
                  <Button onClick={this.showModal}>Enter Contract</Button>
            ...
            {this.state.penis != null && (
              <div>
                <h2>
                  USDC:{" "}
                  {1 /
                    parseFloat(this.state.tokenKeys[0][0].exchange_rate.value)}
                </h2>
                <h2>
                  Tether:{" "}
                  {1 /
                    parseFloat(this.state.tokenKeys[1][0].exchange_rate.value)}
                </h2>
                <h2>
                  Dai:{" "}
                  {1 /
                    parseFloat(this.state.tokenKeys[2][0].exchange_rate.value)}
                </h2>
              </div>
            )}
          </div>
